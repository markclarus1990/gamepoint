...
import { Store, Loader2, Eye, TrendingUp } from "lucide-react";
...
export default function SalesPage() {
  ...
  const fetchSales = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/marketplace/sales?userId=${user.id}`);
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <Link href="/marketplace/create" className="inline-block mt-3 text-sm text-pink-400 hover:text-pink-300">Create a Listing</Link>
          </div>
        ) : (
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <div className="text-sm font-semibold text-white">
                        Sold {tx.points_amount.toLocaleString()} points
                      </div>
                      <div className="text-[10px] text-zinc-500">
                        To <span className="text-zinc-300">{tx.buyer?.name || "Unknown"}</span> — {new Date(tx.completed_at).toLocaleDateString()}
                      </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="text-right">
                      ...
                      <div className="text-[9px] text-zinc-600">{tx.points_amount.toLocaleString()} pts</div>
                    </div>
